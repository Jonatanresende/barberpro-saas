...
const ClientHistoryModal = ({ isOpen, onClose, clientName, appointments }: ClientHistoryModalProps) => {
  ...
  const upcomingAppointments = appointments
    .filter(a => new Date(`${a.data}T${a.hora}`) >= now)
  ...
  const AppointmentList = ({ title, appointments }: { title: string, appointments: Agendamento[] }) => (
    <div>
      <h4 className="text-lg font-semibold text-brand-gold mb-2">{title}</h4>
            ...
            <li key={ag.id} className="bg-brand-gray p-3 rounded-md text-sm">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-gray-400">{new Date(ag.data + 'T00:00:00').toLocaleDateString('pt-BR')} às {ag.hora}</p>
                </div>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Histórico de ${clientName}`}>
      <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2">
        <AppointmentList title="Próximos Agendamentos" appointments={upcomingAppointments} />
        <AppointmentList title="Agendamentos Anteriores" appointments={pastAppointments} />
      </div>
    </Modal>
  );
};
